    ...
    public class xssscanner : iscanner
    {
                        ...
                        while (true)
                        {
                            ...
                            if (helper.urlslist.Count > 0)
                            {
                                ...
                                if (vulnerable)
                                {
                                    ...
                                    Interlocked.Increment(ref helper.stats.xssvulnerable);
                                    me.Add(url);
                                    saver(ref me, false);
                                }
                            }
        ...
        public bool isvulnerable(string url, ref string payload)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < helper.paylodasinstance.xss.Count; i++)
                {
                    if (i >= helper.scannermaxpayloadchecking)
                    {
                        continue;
                    }
                    string currentpayload = helper.paylodasinstance.xss[i];
        ...
        {
            bool res = false;
            for (int i = 0; i < helper.payloadserrorsinstance.xss.Count; i++)
            {
                string currenterror = helper.payloadserrorsinstance.xss[i];
