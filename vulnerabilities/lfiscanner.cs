...
{
    public class lfiscanner : iscanner
    {
                        ...
                        while (true)
                        {
                            ...
                            if (helper.urlslist.Count > 0)
                            {
                                ...
                                if (vulnerable)
                                {
                                    ...
                                    Interlocked.Increment(ref helper.stats.lfivulnerable);
                                    me.Add(url);
                                    saver(ref me, false);
                                }
                            }
        ...
        public bool isvulnerable(string url, ref string payload)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < helper.paylodasinstance.lfi.Count; i++)
                {
                    if (i >= helper.scannermaxpayloadchecking)
                    {
                        continue;
                    }
                    string currentpayload = helper.paylodasinstance.lfi[i];
        ...
        {
            bool res = false;
            for (int i = 0; i < helper.payloadserrorsinstance.lfi.Count; i++)
            {
                string currenterror = helper.payloadserrorsinstance.lfi[i];
