...
{
    public class bingsearcher : isearcher, IDisposable
    {
        public void Dispose()
        {
            GC.Collect();
        ...
        public List<string> getsearch(string dork)
        {
            ...
            try
            {
                ...
                again:
                string url = "https://www.bing.com/search?q=" + dork + "&first=" + currentinheritance;
                string source = req.Get(url, null).ToString();
                List<Match> urls = Regex.Matches(source, @"<a href=""(\S{1,1000})"" h=""ID=\S{1,1000}"">")
                    .Cast<Match>()
                    .ToList();
                ...
                if (urls.Count >= 1)
                {
                    for (int i = 0; i < urls.Count; i++)
                    {
                        ...
                        {
                            res.Add(item);
                            Interlocked.Increment(ref helper.stats.bing);
                        }
                    }
                    currentinheritance += urls.Count + 50;
